 ...
 */
public class ParsedDbcFileParser {
    private static final Logger LOGGER = LoggerFactory.getLogger(ParsedDbcFileParser.class);

    private <T extends DbcEntry> DbcFile getDbcFile(Class<T> template) {
    ...
    public ParsedDbcFile parseFile(String filename, String basePath) throws IOException, IllegalArgumentException {
        ...
        try (DataReader reader = new FileDataReader(file)) {
            ...
            DbcHeader header = new DbcHeaderParser().parse(reader);
            ...
            DbcStringTable stringBlock = new DbcStringTableParser().parse(stringBlockData);
            ...
            ParsedDbcFile parsedDbcFile = new ParsedDbcFile(filename, header, entryData, stringBlock);
            LOGGER.debug(format("[parseFile::%s] Parsed file data in %dms.", filename, timer.stop()));
            return parsedDbcFile;
        }
    }
    ...
    private <T extends DbcEntry> Set<T> parse(Class<T> template, DataReader reader) throws IOException {
        ...
        Set<T> entries = parseEntries(template, entryData, header, stringBlock);
        LOGGER.debug(format("[parse::%s] Parsed type with %d entries in %d ms.", template.getSimpleName(), entries.size(), timer.stop()));
        return entries;
    }

    private <T extends DbcEntry> Set<T> parseEntries(Class<T> template, byte[] data, DbcHeader header, DbcStringTable stringBlock) throws IOException {
    ...
    private <T extends DbcEntry> void populate(Class<T> template, T instance, DataReader reader, DbcStringTable stringBlock) throws IOException {
        ...
        for (int fieldIndex : fields.keySet()) {
            ...
            ByteOrder byteOrder = dbcDataType.getByteOrder(fieldInfo);
            DataType<?> dataType = dbcDataType.getDataType(fieldInfo);
            Object value = reader.readNext(dataType, byteOrder);

            if (fieldInfo.padding()) {
                LOGGER.trace(format("[parse::%s] Ignoring padded field with order %d, type %s[%d] and value '%s'.", template.getSimpleName(), fieldInfo.order(), fieldInfo.dataType(), fieldInfo.numberOfEntries(), value));
    ...
    private String getStringTableReference(int value, DbcStringTable stringBlock) {
        String stringValue;
    ...
    private <T extends DbcEntry> T instantiate(Class<T> template) {
        T instance;
        try {
    ...
    private <T extends DbcEntry> void setValue(Class<T> template, Field field, Object value, T instance, DbcField fieldInfo) {
