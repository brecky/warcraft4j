...
import nl.salp.warcraft4j.clientdata.dbc.util.ref.EntryTypeReferenceInfo;
import nl.salp.warcraft4j.clientdata.dbc.util.ref.FieldReference;
import nl.salp.warcraft4j.clientdata.dbc.util.ref.FieldReferenceInfo;
...
public class EntryReferenceFinder {
    ...
    public Map<Class<? extends DbcEntry>, EntryTypeReferenceInfo> calculateReferences(String dbcDirectory) throws Exception {
        ...
        for (Class<? extends DbcEntry> type : dbcStore.getRegisteredTypes()) {
            EntryTypeReferenceInfo typeRef = new EntryTypeReferenceInfo(type);
            typeRef.scoreFieldReferences(dbcStore);
     ...
     */
    private static void print(Map<Class<? extends DbcEntry>, EntryTypeReferenceInfo> references) {
        if (references.isEmpty()) {
            ...
            for (Map.Entry<Class<? extends DbcEntry>, EntryTypeReferenceInfo> r : references.entrySet()) {
                Class<? extends DbcEntry> type = r.getKey();
