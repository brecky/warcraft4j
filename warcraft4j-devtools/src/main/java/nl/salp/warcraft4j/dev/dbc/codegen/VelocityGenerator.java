...
public abstract class VelocityGenerator<T> {
    ...
    private File generate(T instance, Template template) throws IOException {
        ...
        if (outputFile.exists() && !overrideFiles) {
            getLogger().debug(format("Skipping existing file %s for %s instance %s", outputFile.getPath(), instance.getClass().getName(), instance));
            output = null;
        } else {
            if (outputFile.exists()) {
                getLogger().debug(format("Overwriting existing file %s for %s instance %s", outputFile.getPath(), instance.getClass().getName(), instance));
            }
            try (FileWriter writer = new FileWriter(outputFile, false)) {
                ...
                writer.flush();
                output = outputFile;
                getLogger().debug(format("Generated file %s from %s instance %s", outputFile.getPath(), instance.getClass().getName(), instance));
            } catch (IOException e) {
