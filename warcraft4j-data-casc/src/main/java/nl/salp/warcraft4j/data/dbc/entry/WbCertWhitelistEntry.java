 ...
 */
@DbcMapping(file = "WbCertWhitelist.db2")
public class WbCertWhitelistEntry implements DbcEntry {
    private static final DbcType ENTRY_TYPE = DbcType.WB_CERT_WHITELIST;

    @DbcFieldMapping(order = 1, dataType = DbcDataType.UINT32)
    private int id;
    @DbcFieldMapping(order = 2, dataType = DbcDataType.STRINGTABLE_REFERENCE)
    private String host;
    @DbcFieldMapping(order = 3, dataType = DbcDataType.BYTE, numberOfEntries = 2, knownMeaning = false)
    private byte[] unknown3;
    @DbcFieldMapping(order = 4, dataType = DbcDataType.BYTE, numberOfEntries = 2, knownMeaning = false)
