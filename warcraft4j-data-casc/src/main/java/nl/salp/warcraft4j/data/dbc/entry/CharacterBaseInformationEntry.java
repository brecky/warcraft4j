 ...
 */
@DbcMapping(file = "CharBaseInfo.dbc")
public class CharacterBaseInformationEntry implements DbcEntry {
    private static final DbcType ENTRY_TYPE = DbcType.CHARACTER_BASE_INFORMATION;
    @DbcFieldMapping(order = 1, dataType = DbcDataType.UINT32)
    private int id;
    @DbcFieldMapping(order = 2, dataType = DbcDataType.BYTE)
    private byte raceId;
    @DbcFieldMapping(order = 3, dataType = DbcDataType.BYTE)
    private byte characterClassId;
    @DbcFieldMapping(order = 4, dataType = DbcDataType.BYTE, numberOfEntries = 2, padding = true)
