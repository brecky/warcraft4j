 ...
 */
@DbcFile(file = "WbCertWhitelist.db2")
public class WbCertWhitelistEntry implements ClientDatabaseEntry {
    private static final ClientDatabaseEntryType ENTRY_TYPE = ClientDatabaseEntryType.WB_CERT_WHITELIST;

    @DbcField(order = 1, dataType = DbcDataType.UINT32)
    private int id;
    @DbcField(order = 2, dataType = DbcDataType.STRINGTABLE_REFERENCE)
    private String host;
    @DbcField(order = 3, dataType = DbcDataType.BYTE, numberOfEntries = 2, knownMeaning = false)
    private byte[] unknown3;
    @DbcField(order = 4, dataType = DbcDataType.BYTE, numberOfEntries = 2, knownMeaning = false)
