 ...
 */
@DbcFile(file = "ItemSet.dbc")
public class ItemSetEntry implements ClientDatabaseEntry {
    private static final ClientDatabaseEntryType ENTRY_TYPE = ClientDatabaseEntryType.ITEM_SET;

    @DbcField(order = 1, dataType = DbcDataType.UINT32)
    private int id;
    @DbcField(order = 2, dataType = DbcDataType.STRINGTABLE_REFERENCE)
    private String name;
    @DbcField(order = 3, dataType = DbcDataType.UINT32, numberOfEntries = 17)
    @DbcReference(type = ClientDatabaseEntryType.ITEM)
    private int[] itemIds;
    @DbcField(order = 4, dataType = DbcDataType.UINT32)
    // TODO Reference to the trade skill...
