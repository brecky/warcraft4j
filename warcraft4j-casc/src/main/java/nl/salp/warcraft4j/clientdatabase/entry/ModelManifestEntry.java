 ...
 */
@DbcFile(file = "ModelManifest.db2")
public class ModelManifestEntry implements ClientDatabaseEntry {
    private static final ClientDatabaseEntryType ENTRY_TYPE = ClientDatabaseEntryType.MODEL_MANIFEST;
    ...
    /** FIXME The header indicates 7 fields with a total size of 32 bytes per entry, but since there is no data in the file, we can't analyse which field is 64-bit. */
    @DbcField(order = 8, dataType = DbcDataType.UINT32, knownMeaning = false, padding = true)
    private transient int unknown8;
